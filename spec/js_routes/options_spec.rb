...
describe JsRoutes, "options" do
  ...
  describe "serializer" do
    it "allows to extend serializer" do
      ...
      JS
      expectjs("Routes.inboxes_path({a: [{b: 1}, {c: undefined}, {d: null}]})").to eql("/inboxes?a%5B%5D%5Bb%5D=1&&")
    end
    ...
    context "when specified" do
      ...
      # for all inputs
      let(:_presetup) { 'function myCustomSerializer(object, prefix) { return "foo=bar"; }' }
      let(:_options) { {serializer: "myCustomSerializer"} }

      it "sets configurable serializer" do
        # expect the nonsense serializer above to have appened foo=bar
        # to the end of the path
        expectjs("Routes.inboxes_path()").to eql("/inboxes?foo=bar")
      end
    end

    context "when specified, but not function" do
      let(:_presetup) { "var myCustomSerializer = 1" }
      let(:_options) { {serializer: "myCustomSerializer"} }

      it "throws error" do
        expect {
          evaljs("Routes.inboxes_path({a: 1})")
        }.to raise_error(js_error_class)
      end
    end

    context "when configured in js" do
      let(:_options) { {serializer: 'function (object, prefix) { return "foo=bar"; }'} }

      it "uses JS serializer" do
        evaljs("Routes.configure({serializer: function (object, prefix) { return 'bar=baz'; }})")
        expectjs("Routes.inboxes_path({a: 1})").to eql("/inboxes?bar=baz")
      end
    end
  end
  ...
  context "when exclude is specified" do
    ...
    it "excludes specified routes from file" do
      expectjs("Routes.admin_users_path").to be_nil
    end

    it "does not exclude routes not under specified pattern" do
    ...
    context "for rails engine" do
      ...
      it "excludes specified engine route" do
  ...
  context "when include is specified" do
    ...
    context "for rails engine" do
      ...
      it "includes specified engine route" do
  ...
  describe "prefix option" do
    ...
    context "with trailing slash is specified" do
      ...
      it "renders routing with prefix" do
        expectjs("Routes.inbox_path(1)").to eq("/myprefix#{test_routes.inbox_path(1)}")
      end

      it "renders routing with prefix set in JavaScript" do
    ...
    context "combined with url links and default_url_options" do
      ...
      it "generates path and url links" do
  ...
  context "when default format is specified" do
    ...
    it "renders routing with default_format" do
      expectjs("Routes.inbox_path(1)").to eq(test_routes.inbox_path(1))
    end

    it "renders routing with default_format and zero object" do
      expectjs("Routes.inbox_path(0)").to eq(test_routes.inbox_path(0))
    end

    it "overrides default_format when spefified implicitly" do
      expectjs("Routes.inbox_path(1, {format: 'xml'})").to eq(test_routes.inbox_path(1, format: "xml"))
    end

    it "overrides nullify implicitly when specified implicitly" do
  ...
  it "does not include the format when {:format => false} is specified" do
  ...
  describe "default_url_options" do
    context "with optional route parts" do
      context "provided by the default_url_options" do
        ...
        it "uses this options to fill optional parameters" do
      ...
      context "provided inline by the method parameters" do
        ...
        it "overwrites the default_url_options" do
    ...
    context "with optional and required route parts" do
      ...
      it "uses this options to fill the optional parameters" do
  ...
  describe "camel_case" do
    ...
    context "with true" do
      ...
      it "generates camel case routes" do
  ...
  describe "url_links" do
    context "with default option" do
      ...
      it "generates only path links" do
        expectjs("Routes.inbox_path(1)").to eq(test_routes.inbox_path(1))
