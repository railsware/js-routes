(function() {
  ...
  Utils = {
      ...
      for (key in obj) {
        ...
        if (this.getObjectType(prop) === "array") {
          ...
          for (i = _i = 0, _len = prop.length; _i < _len; i = ++_i) {
            val = prop[i];
            if (this.getObjectType(val) === "object") {
              s.push(this.serialize(val, key));
            } else {
              s.push(this.encode_parameter(key, val));
            }
          }
        } else {
          if (this.getObjectType(prop) === "object") {
            s.push(this.serialize(prop, key));
          } else {
            s.push(this.encode_parameter(key, prop));
          }
        }
      }
      if (!s.length) {
    ...
    encode_parameter: function(key, value) {
      return "" + (encodeURIComponent(key.toString())) + "=" + (encodeURIComponent(value.toString()));
    },
